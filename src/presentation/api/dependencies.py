...
async def get_admin_dashboard_use_case(
    ...
    order_repo = OrderRepositoryImpl(OrderDAO(session))
    return GetAdminDashboardUseCase(user_repo=user_repo, order_repo=order_repo)
...
def verify_webapp_signature(role: UserRole) -> Callable:
    async def decorator(x_telegram_init_data: str = Header(...)) -> dict:
        ...
        # Используем токен конкретного бота на основе переданной роли
