...
from src.core.application.use_cases.dtos.order_dtos import OrderDTO, OrderPaymentUpdateDTO
from src.core.application.exceptions.not_found_exception import NotFoundException


class PatchOrderPaymentUseCase:
    def __init__(self, order_repo: OrderRepository):
        self.order_repo = order_repo
    
    async def execute(self, order_payment_input_dto: OrderPaymentUpdateDTO) -> OrderDTO:
        order_entity = await self.order_repo.get_by_id(order_id=order_payment_input_dto.order_id)
        if not order_entity:
            raise NotFoundException(f"Order with id {order_payment_input_dto.order_id} not found")
        
        order_entity.update_payment_info(
            payment_method=order_payment_input_dto.payment_method,
        )
