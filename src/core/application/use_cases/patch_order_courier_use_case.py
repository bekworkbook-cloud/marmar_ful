...
class PatchOrderCourierUseCase:
    ...
    async def execute(self, order_personnel_update_dto: OrderPersonnelUpdateDTO) -> OrderDTO:

        order_entity = await self.order_repo.get_by_id(order_id=order_personnel_update_dto.order_id)
        if not order_entity:
            raise NotFoundException(f"Order with id {order_personnel_update_dto.order_id} not found")
